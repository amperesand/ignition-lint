...
| `PERFORMANCE_CONSIDERATION` | INFO | Large static table/repeater data rendered directly in a component |
...
| `EXPR_NOW_LOW_POLLING` | INFO | `now(N)` where 0 < N < 1 000 ms — may impact client performance |
...
| `JYTHON_MIXED_INDENTATION` | STYLE | Mixed tabs and spaces on the same line |
...
| `JYTHON_BAD_COMPONENT_REF` | STYLE | Positional/dynamic component traversal, `getParent`, or legacy `getComponent` usage |
